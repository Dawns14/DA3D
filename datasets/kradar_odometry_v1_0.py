...
def matrix_to_quaternion_and_position(matrix):
    R = torch.tensor(matrix[:3, :3])
    pos = matrix[:3, 3]
    q = transforms.matrix_to_quaternion(R).detach().cpu().numpy()
    return q, pos
...
class KRadarOdometry_v1_0(Dataset):
    def __init__(self, cfg=None, split='all'):
        ...
        self.load_label_in_advance = True
        ...
        self.collate_ver = self.cfg.get('collate_fn', 'v2_1')
    ...
    def get_dict_cam_calib_from_yml(self):
        ...
        for yml_file_name in list_yml:
            ...
            dict_cam_calib[key_name] = get_matrices_from_dict_calib(dict_temp)
        return dict_cam_calib

    def get_dict_cam_calib_from_npy(self, dict_item):
        ...
        for npy_file_name in list_npy:
            ...
            if key_name == 'cam2pix':
                ...
                intrinsic.append(temp[:3, :3])
            elif key_name == 'ldr2cam':
                ldr2cam.append(temp)
                temp_inv = np.linalg.inv(temp)
                cam2ldr.append(temp_inv)

        for i in range(len(intrinsic)):
            lidar2image = intrinsic[i] @ ldr2cam[i][:3, :4]
            ldr2img.append(lidar2image)

        dict_item['camera_intrinsics'] = intrinsic
        dict_item['camera2lidar'] = cam2ldr
        dict_item['lidar2image'] = ldr2img
    ...
    def collate_fn(self, list_batch):
        ...
        if self.collate_ver == 'v2_1':
            ...
            list_sparse_keys = ['rdr_sparse', 'ldr64', 'pc100p', 'pc30p', 'pc20p', 'pc15p', 'pc10p', 'pc5p', 'pc1p',\
                                'rdr_sparse_cons', 'rdr_sparse_idx_t']
            ...
            for batch_idx in range(batch_size):
                ...
                if dict_batch['num_objs'][batch_idx] == 0:
                    continue
