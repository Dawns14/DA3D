...
from datasets.sampler import ClimateBatchSampler
import cv2
...
class KRadarDetection_v2_0(Dataset):
    ...
    def weather_len(self, weather):
        cnt = 0
        for item in self.list_dict_item:
            if self.get_description(item)['meta']['desc']['climate'] == weather:
                cnt += 1
        return cnt

    def sort_and_batch_by_weather(self, list_dict_item):
        climate_groups = {}
        for item in list_dict_item:
            climate = self.get_description(item)['meta']['desc']['climate']
            if climate not in climate_groups:
                climate_groups[climate] = []
            climate_groups[climate].append(item)

        climate_batches = []
        for climate, items in climate_groups.items():
            random.shuffle(items)
            for i in range(0, len(items), self.batch_size):
                batch = items[i:i + self.batch_size]
                if len(batch) < self.batch_size:
                    batch += random.choices(items, k=self.batch_size - len(batch))
                climate_batches.append(batch)

        random.shuffle(climate_batches)

        list_dict_item = [sample for batch in climate_batches for sample in batch]
    ...
    def load_dict_item(self, path_data, split):
        ...
        for seq, path_header in list_seqs_w_header:
            ...
            for label in list_labels:
                ...
                path_label_v2_0 = osp.join(f'{path_data.revised_label_v2_0}', seq, label)

                path_label_v2_1 = osp.join(f'{path_data.revised_label_v2_1}', seq, label)
        ...
        if self.cfg.get('filter_weather', None) != None and split=='train':
            list_dict_item = list(filter(lambda item: self.get_description(item)['meta']['desc']['climate']==self.cfg.filter_weather, list_dict_item))

        if self.cfg.get('sort_by_weather', False) and split=='train':

            self.batch_sampler = ClimateBatchSampler(list_dict_item, self.batch_size, self.get_description)
    ...
    def get_label(self, dict_item):
        ...
        dict_path = dict(
            ...
            cam_front_img = osp.join(header, seq, 'cam-front', f'cam-front_{camf}.png')
        )
    ...
    def get_calib_values(self, path_calib):
        ...
        list_values = [list_calib[1], list_calib[2], self.calib['z_offset']]
        return list_values
    ...
    def get_description(self, dict_item):
    ...
    def get_img(self, dict_item):
        img_path = dict_item['meta']['path']['cam_front_img']
        img = cv2.imread(img_path)[:, :1280]
        transform = transforms.Compose(
            [transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        img = transform(img)
        dict_item['cam_front_img'] = img
    ...
    def __getitem__(self, idx):
        ...
        dict_item = self.get_img(dict_item) if self.item.get('cam_front_img', False) else dict_item
        ...
        if self.collate_ver == 'v1_0':
            ...
            for batch_idx, dict_item in enumerate(list_batch):
                for k, v in dict_item.items():
                    ...
                    elif k in ['rdr_sparse', 'ldr64', 'cam_front_img']:
                        if isinstance(dict_item[k], np.ndarray):
                            dict_batch[k].append(torch.from_numpy(dict_item[k]).float())
                        elif isinstance(dict_item[k], torch.Tensor):
                            dict_batch[k].append(dict_item[k].float())
            dict_batch['batch_size'] = batch_idx+1

            for k in list_keys:
                if k in ['rdr_sparse', 'ldr64', 'cam_front_img']:
                    batch_indices = []
            ...
            for batch_idx, dict_item in enumerate(list_batch):
                for k, v in dict_item.items():
                    ...
                    elif k in ['rdr_sparse', 'ldr64', 'de_ldr64', 'cam_front_img']:
                        if isinstance(dict_item[k], np.ndarray):
                            dict_batch[k].append(torch.from_numpy(dict_item[k]).float())
                        elif isinstance(dict_item[k], torch.Tensor):
                            dict_batch[k].append(dict_item[k].float())
