...
class ResNetFPN(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.MODEL.BACKBONE.FPN_MODE == 'bilinear':
            ...
            for idx_rate, rate in enumerate(cfg.MODEL.BACKBONE.BIFPN_RATE):
                if idx_rate != 0:
                    self.list_downsample_func.append(nn.MaxPool2d(kernel_size=2, stride=2))
                if rate == 1:
                    upsample_func = nn.ConvTranspose2d(bi_fpn_in_channels,\
                        bi_fpn_out_channels[idx_rate], 3, rate, padding=1)
                elif rate == 2:
                    upsample_func = nn.ConvTranspose2d(bi_fpn_in_channels,\
                        bi_fpn_out_channels[idx_rate], 3, rate, padding=1, output_padding=(1,1))
                elif rate == 4:
                    upsample_func = nn.ConvTranspose2d(bi_fpn_in_channels,\
                        bi_fpn_out_channels[idx_rate], 3, rate, padding=1, output_padding=(3,3))
    ...
    def forward(self, dict_item):
        ...
        out_feat = getattr(self, f'fpn_mode_{self.fpn_mode}')(list_feat)
        dict_item['out_feat'] = out_feat

        return dict_item

    def fpn_mode_0(self, list_feat):
        list_upsampled = []
    ...
    def fpn_mode_2(self, list_feat):
        ...
        list_bifpn_feat = []
        for upsample_func, feat in zip(self.list_bifpn_upsample_func, list_fpn_feat):
            upsampled = upsample_func(feat)
            list_bifpn_feat.append(upsampled)

        return torch.cat(list_bifpn_feat, dim=1)
