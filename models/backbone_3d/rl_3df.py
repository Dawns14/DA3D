    ...
    def forward(self, dict_item):
        ...
        for idx_layer in range(self.num_layer):
            ...
            if self.is_z_embed:
                ...
                bev_spR = getattr(self, f'toBEV{idx_layer}R')(xL2)
                ...
                bev_dense_spR = bev_spR.dense()
                bev_denseR = torch.max(bev_dense_spR, dim=2)[0] if bev_dense_spR.shape[2] > 1 else bev_dense_spR.squeeze(2)
                bev_denseR = getattr(self, f'convtrans2d{idx_layer}R')(bev_denseR)

                bev_dense_spL = bev_spL.dense()
                bev_denseL = torch.max(bev_dense_spL, dim=2)[0] if bev_dense_spL.shape[2] > 1 else bev_dense_spL.squeeze(2)
                bev_denseL = getattr(self, f'convtrans2d{idx_layer}L')(bev_denseL)
