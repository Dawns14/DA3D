...
class interRAL(nn.Module):
    ...
    def forward(self, x, y):
        ...
        energy = torch.bmm(x_q, y_k)
        attention = self.softmax(energy)
        attention = attention / (1e-9 + attention.sum(dim=1, keepdims=True))
        y_r = torch.bmm(attention, y_v).permute(1, 2, 0)
...
class BiDF_PillarVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        for i in range(len_radar):
            ...
            com_features[:, i, now_feature_idx : now_feature_idx + 3] = radar_voxel_features[:, i, :3]
            now_feature_idx += 3
            ...
            com_features[valid_common_R, i, now_feature_idx : now_feature_idx + 1] = extraF_L
            now_feature_idx += 1
            ...
            com_features[valid_common_R, i, now_feature_idx : now_feature_idx + radartolidar_f_cluster.shape[-1]] = radartolidar_f_cluster
            now_feature_idx += radartolidar_f_cluster.shape[-1]

            com_features[:, i, now_feature_idx : now_feature_idx + radar_f_center.shape[-1]] = radar_f_center[:, i]
            now_feature_idx += radar_f_center.shape[-1]

            com_features[:, i, now_feature_idx : now_feature_idx + radar_f_cluster.shape[-1]] = radar_f_cluster[:, i]
        ...
        now_feature_idx = 0
        l_ex_features[:, :, now_feature_idx : now_feature_idx + lidar_voxel_features.shape[-1]] = lidar_voxel_features
        now_feature_idx += lidar_voxel_features.shape[-1]

        l_ex_features[:, :, now_feature_idx : now_feature_idx + lidar_f_cluster.shape[-1]] = lidar_f_cluster
        now_feature_idx += lidar_f_cluster.shape[-1]

        l_ex_features[:, :, now_feature_idx : now_feature_idx + lidar_f_center.shape[-1]] = lidar_f_center
        ...
        l_ex_features[l2r_com_L, :, now_feature_idx : now_feature_idx + lidartoradar_f_cluster.shape[-1]] = lidartoradar_f_cluster
...
class Attention_Layer(nn.Module):
    ...
    def forward(self, inputs, Q_in, input_coords, Q_in_coords, fog_intensity = None):
        outs = []
        Q_in = Q_in.reshape(1, -1, Q_in.shape[-1])
