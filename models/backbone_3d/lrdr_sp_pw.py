...
class LRSparseBackbone(nn.Module):
    def __init__(self, cfg):
        super(LRSparseBackbone, self).__init__()
        self.cfg = cfg
        ...
        input_dim = cfg_model.BACKBONE.INPUT_DIM
        ...
        if cfg_model.BACKBONE.TO_BEV.IS_Z_EMBED:
            ...
            for idx_bev in range(self.num_layer):
                ...
                setattr(self, f'l_toBEV{idx_bev}',\
                    spconv.SparseConv3d(in_channels=temp_enc_ch,\
                        out_channels=temp_enc_ch, kernel_size=(z_kernel_size, 1, 1)))
                setattr(self, f'l_bnBEV{idx_bev}',\
                    nn.BatchNorm1d(temp_enc_ch))
                setattr(self, f'l_convtrans2d{idx_bev}',\
                    nn.ConvTranspose2d(in_channels=temp_enc_ch, out_channels=temp_out_channel,\
                        kernel_size=list_bev_kernel[idx_bev], stride=list_bev_stride[idx_bev],  padding=list_bev_padding[idx_bev]))
                setattr(self, f'l_bnt{idx_bev}', nn.BatchNorm2d(temp_out_channel))
                setattr(self, f'r_toBEV{idx_bev}',\
                    spconv.SparseConv3d(in_channels=temp_enc_ch,\
                        out_channels=temp_enc_ch, kernel_size=(z_kernel_size, 1, 1)))
                setattr(self, f'r_bnBEV{idx_bev}',\
                    nn.BatchNorm1d(temp_enc_ch))
                setattr(self, f'r_convtrans2d{idx_bev}',\
                    nn.ConvTranspose2d(in_channels=temp_enc_ch, out_channels=temp_out_channel,\
                        kernel_size=list_bev_kernel[idx_bev], stride=list_bev_stride[idx_bev],  padding=list_bev_padding[idx_bev]))
                setattr(self, f'r_bnt{idx_bev}', nn.BatchNorm2d(temp_out_channel))
    ...
    def forward(self, dict_item):
        ...
        dict_item['bev_feat'] = bev_features
