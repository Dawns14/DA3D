...
class ImgDataset(Dataset):
    ...
    def get_data_indices(self, path_label):
        ...
        rdr_idx, ldr_idx, camf_idx, _, camr_idx = line.split(',')[0].split('=')[1].split('_')

        return seq_id, rdr_idx, ldr_idx, camf_idx, camr_idx

    def __getitem__(self, idx):
        path_label = self.list_path_label[idx]
        seq_id, radar_idx, lidar_idx, camf_idx, camr_idx = self.get_data_indices(path_label)
        path_header = path_label.split('/')[:-2]
        path_cam_front = '/' + os.path.join(*path_header, 'cam-front', 'cam-front_' + camf_idx + '.png')
        path_cam_left = '/' + os.path.join(*path_header, 'cam-left', 'cam-left_' + camr_idx + '.png')
        path_cam_right = '/' + os.path.join(*path_header, 'cam-right', 'cam-right_' + camr_idx + '.png')
...
path_cfg = '/home/joson/personal/pro/3d_lidar/new/K-Radar/configs/train_weather_cls.yml'
...
for epoch in range(100):
    ...
    if acc > best_acc:
        torch.save(network.state_dict(),
                   '/home/joson/personal/pro/3d_lidar/new/K-Radar/models/img_cls/best' + str(epoch).zfill(3) + '_' + str(acc*100).split('.')[0] + '.pth')
