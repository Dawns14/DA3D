...
class RadarSparseProcessor_L(nn.Module):
    ...
    def forward(self, dict_item):
        if self.cfg_dataset_ver2:
            ldr_sparse = dict_item['ldr64'].cuda()
            batch_indices = dict_item['batch_indices_ldr64'].cuda()
            ...
            for batch_idx in range(dict_item['batch_size']):
                ...
                vox_in = ldr_sparse[corr_ind[0],:]

                voxel_features, voxel_coords, voxel_num_points = self.gen_voxels(vox_in)
                voxel_batch_idx = torch.full((voxel_coords.shape[0], 1), batch_idx, device=ldr_sparse.device, dtype=torch.int64)
                voxel_coords = torch.cat((voxel_batch_idx, voxel_coords), dim=-1)
            ...
            dict_item['sp_features_l'] = voxel_features.contiguous()
            dict_item['sp_indices_l'] = voxel_coords.int()
        else:
            if self.type_data == 0:
