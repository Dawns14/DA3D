...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, kv):
        ...
        kv = self.kv(kv).reshape(B, T, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class Denosier(nn.Module):
    ...
    def forward(self, feat, noisy_masks, upsam=False, t=None):
        ...
        kv = cur_feat.reshape(B, C, -1).transpose(1,2)
...
class Cond_Diff_Denoise(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            x = combined_pred
            latent_shape = x.shape
            x_start = spatial_features * self.signal_scaling_rate
            noise = default(None, lambda: torch.randn_like(x_start))
