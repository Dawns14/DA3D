...
def func_show_radar_cube_bev(p_pline, dict_item, bboxes=None, magnifying=4, is_with_doppler = False, is_with_log = False):
    ...
    for bbox in bboxes:
        ...
        rdr_cube_bev_vis = cv2.circle(rdr_cube_bev_vis, (idx_x,idx_y),1,(0,0,255),thickness=-1)
    ...
    for idx_item in tqdm(range(len(p_ds.list_dict_item))):
        ...
        for idx_cam, key_cam in enumerate(dict_option.list_process_cams):
            ...
            if prev_seq[idx_cam] != int(seq):
                ...
                T_cam2sen = np.linalg.inv(T_sen2cam)
                T_sen2pix = T_cam2pix@T_sen2cam
            ...
            img_undistorted = Image.fromarray(np.flip(img_temp, axis=2))
            ...
            if vis:
                if to_radar_coord:
                    ...
                    ldr_points = p_ds.get_ldr64_from_path(dict_path['ldr64'], is_calib=False)
