...
def test_box_intersection_th():
    ...
    tensor1 = torch.FloatTensor(np.stack([corners1, corners2], axis=0))
...
def test_area():
    ...
    corners1 = box2corners(*box1)
    corners2 = box2corners(*box2)
    tensor1 = torch.FloatTensor(np.stack([corners1, corners1], axis=0))
    tensor1 = torch.unsqueeze(tensor1, 0).repeat([2,1,1,1]).cuda()
    tensor2 = torch.FloatTensor(np.stack([corners2, corners1], axis=0))
